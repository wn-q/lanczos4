...
localparam [IMG_X_W-1:0] X_KEEP_PIX_IDX = X_KEEP_PIX;
...
// 模块内部按 0,-1,-2,-3,+1,+2,+3,+4 的顺序生成 x/y 方向各 8 个 tap。
...
//根据输出读的数据
assign linebuf_rd_data_mux = linebuf_dout[linebuf_rd_bank];
...
assign lanczos_window_valid = &lanczos_window_valid_mask_r;
...
wire       tap_read_active;
wire [8:0] cur_write_len_full;
wire [4:0] cur_write_len;

assign in_block_x_base   = {seg16_x, 4'b0000}; // 当前16像素段在block内部的x起点，等于seg16_x*16
assign in_global_x_base  = block_start_x + {5'd0, seg16_x, 4'b0000}; // 当前16像素段在整帧图像中的x起点
assign linebuf_wr_row_mod= row_cnt % 7'd7; // line SRAM只有7个bank，row7覆盖bank0，row8覆盖bank1
assign linebuf_wr_row    = linebuf_wr_row_mod[2:0]; // 当前输入行将要写入/覆盖的line SRAM bank编号
...
assign right_save_en = !frame_right_edge;  // 不是frame右边界的block需要保存右7列到right_buffer供右侧block使用
...
reg [7:0]  calc_block_x_base;      // 当前16像素段在block内部的x起点
reg [12:0] calc_global_x_base;     // 当前16像素段在整幅图中的x起点
reg [2:0]  calc_linebuf_row;       // 当前计算行对应的line SRAM bank编号0-6
reg [6:0]  calc_row_cnt;           // 当前计算行在block内部的真实行号0~(block_pixel_height-1)
reg        calc_first_seg_in_row;  // 当前段是否是本行第一个16像素段
reg        calc_last_seg_in_row;   // 当前段是否是本行最后一个16像素段
reg        calc_last_row_in_block; // 当前段是否属于block最后一行

assign evict_right_row     = calc_row_cnt - 7'd7;  //当前正在写回的这一行，会覆盖 line SRAM 里面 7 行前的旧历史行
assign evict_right_row_idx = evict_right_row[4:0]; //这条旧历史行应该写入 right_buffer 的哪一行

// ---------------------------------------------------------------------------
// Common tap read rule for the 8-tap row interface.
//根据lanczos坐标读取对应的像素值
// 返回格式：
//   [PIXEL_W-1:0] : tap像素值
//   [PIXEL_W]     : tap是否有效
//   [PIXEL_W+1]   : tap是否来自左侧block保留下来的right_buffer
// ---------------------------------------------------------------------------
function [PIXEL_W+1:0] read_lanczos_tap;
    input signed [TAP_COORD_W-1:0] tap_x_g;
    input signed [TAP_COORD_W-1:0] tap_y_g;

    reg [PIXEL_W-1:0] f_pixel;
    reg               f_valid;
    reg               f_from_right;

    reg signed [TAP_COORD_W-1:0] f_clip_x_g;
    reg signed [TAP_COORD_W-1:0] f_clip_y_g;
    reg signed [TAP_COORD_W-1:0] f_local_x;
    reg signed [TAP_COORD_W-1:0] f_local_y;
    reg [12:0]        f_clip_x_u;
    reg [6:0]         f_local_y_u;
    reg [6:0]         f_linebuf_row_mod;
    reg [2:0]         f_linebuf_row;
    reg [IMG_X_W-1:0] f_global_x_idx;

    reg signed [TAP_COORD_W-1:0] f_frame_width_s;
    reg signed [TAP_COORD_W-1:0] f_frame_height_s;
    reg signed [TAP_COORD_W-1:0] f_block_start_x_s;
    reg signed [TAP_COORD_W-1:0] f_block_start_y_s;
    reg signed [TAP_COORD_W-1:0] f_block_height_s;
    reg signed [TAP_COORD_W-1:0] f_calc_row_cnt_s;
    reg signed [TAP_COORD_W-1:0] f_tap_y_min_s;
    reg signed [TAP_COORD_W-1:0] f_block_width_s;
    reg signed [TAP_COORD_W-1:0] f_calc_x_base_s;
    reg signed [TAP_COORD_W-1:0] f_calc_x_limit_s;
    reg signed [TAP_COORD_W-1:0] f_calc_left7_base_s;
    reg signed [TAP_COORD_W-1:0] f_right_idx_full;
    reg signed [TAP_COORD_W-1:0] f_cur16_idx_full;
    reg signed [TAP_COORD_W-1:0] f_left7_idx_full;
    reg [2:0]         f_right_idx;
    reg [3:0]         f_cur16_idx;
    reg [2:0]         f_left7_idx;
    reg               f_tap_y_in_block;
    reg               f_tap_y_in_window;
begin
    f_pixel = {PIXEL_W{1'b0}};
    f_valid = 1'b0;
    f_from_right = 1'b0;

    f_frame_width_s = {1'b0, sw_upscale_pic_width};
    f_frame_height_s = {1'b0, sw_pic_height};
    f_block_start_x_s = {1'b0, block_start_x};
    f_block_start_y_s = {1'b0, block_start_y};

    // Lanczos给出的是全局tap坐标，先做frame边界clip，再映射到当前block内部坐标。
    f_clip_x_g = tap_x_g;
    if (frame_left_edge && (tap_x_g < TAP_ZERO)) begin
        f_clip_x_g = TAP_ZERO;
    end else if (frame_right_edge && (tap_x_g >= f_frame_width_s)) begin
        f_clip_x_g = f_frame_width_s - TAP_ONE;
    end

    f_clip_y_g = tap_y_g;
    if (frame_top_edge && (tap_y_g < TAP_ZERO)) begin
        f_clip_y_g = TAP_ZERO;
    end else if (frame_bottom_edge && (tap_y_g >= f_frame_height_s)) begin
        f_clip_y_g = f_frame_height_s - TAP_ONE;
    end

    f_local_x = f_clip_x_g - f_block_start_x_s;
    f_local_y = f_clip_y_g - f_block_start_y_s;

    f_clip_x_u = f_clip_x_g[12:0];
    f_local_y_u = f_local_y[6:0];
    f_linebuf_row_mod = f_local_y_u % 7'd7;
    f_linebuf_row = f_linebuf_row_mod[2:0];
    f_global_x_idx = f_clip_x_u[IMG_X_W-1:0];

    f_block_height_s = {7'b0, block_pixel_height};
    f_calc_row_cnt_s = {7'b0, calc_row_cnt};
    f_tap_y_min_s = (calc_row_cnt >= 7'd7) ? ({7'b0, calc_row_cnt} - TAP_SEVEN) : TAP_ZERO;
    f_tap_y_in_block = (f_local_y >= TAP_ZERO) && (f_local_y < f_block_height_s);
    f_tap_y_in_window = (f_local_y >= f_tap_y_min_s) && (f_local_y <= f_calc_row_cnt_s);

    f_block_width_s = {6'b0, block_pixel_width};
    f_calc_x_base_s = {6'b0, calc_block_x_base};
    f_calc_x_limit_s = {6'b0, calc_block_x_base} + TAP_SIXTEEN;
    f_calc_left7_base_s = {6'b0, calc_block_x_base} - TAP_SEVEN;

    f_right_idx_full = f_local_x + TAP_SEVEN;
    f_cur16_idx_full = f_local_x - f_calc_x_base_s;
    f_left7_idx_full = f_local_x - f_calc_left7_base_s;
    f_right_idx = f_right_idx_full[2:0];
    f_cur16_idx = f_cur16_idx_full[3:0];
    f_left7_idx = f_left7_idx_full[2:0];

    if (tap_read_active && f_tap_y_in_block && f_tap_y_in_window) begin
        if (f_local_x < TAP_ZERO) begin
            if (!frame_left_edge && (f_local_x >= -TAP_SEVEN)) begin
                f_pixel = right_buffer[f_local_y_u[4:0]][f_right_idx];
                f_valid = 1'b1;
                f_from_right = 1'b1;
            end
        end else if (f_local_x < f_block_width_s) begin
            if ((f_local_y == f_calc_row_cnt_s) &&
                (f_local_x >= f_calc_x_base_s) &&
                (f_local_x < f_calc_x_limit_s)) begin
                // 当前16像素段尚未写回line SRAM，直接读cur16_reg。
                f_pixel = cur16_reg[f_cur16_idx];
                f_valid = 1'b1;
            end else if ((f_local_y == f_calc_row_cnt_s) &&
                         !calc_first_seg_in_row &&
                         (f_local_x >= f_calc_left7_base_s) &&
                         (f_local_x < f_calc_x_base_s)) begin
                // 当前行上一段的最后7个像素尚未写回line SRAM，直接读left7_reg。
                f_pixel = left7_reg[f_left7_idx];
                f_valid = 1'b1;
            end else begin
                // line_buffer已经替换为同步SRAM，历史行不能再组合读取。
                // 后续需要接入多拍window read状态机。
                f_pixel = {PIXEL_W{1'b0}};
                f_valid = 1'b0;
            end
        end
    ...
    read_lanczos_tap = {f_from_right, f_valid, f_pixel};
end
endfunction

// ---------------------------------------------------------------------------
// Lanczos 8x8 window read mux.
//
// Lanczos给出原图整数中心点，模块按0,-1,-2,-3,+1,+2,+3,+4的顺序
// 同时生成x/y方向各8个tap，组合成64个像素窗口。
function signed [TAP_COORD_W-1:0] tap_offset_by_idx;
    input [2:0] tap_idx;
begin
    case (tap_idx)
        3'd0: tap_offset_by_idx = TAP_ZERO;
        3'd1: tap_offset_by_idx = -TAP_ONE;
        3'd2: tap_offset_by_idx = -TAP_TWO;
        3'd3: tap_offset_by_idx = -TAP_THREE;
...
reg signed [TAP_COORD_W-1:0] window_tap_x_i;
reg signed [TAP_COORD_W-1:0] window_tap_y_i;
reg [PIXEL_W+1:0] window_tap_result_i;
integer win_x_i;
integer win_y_i;
integer win_idx_i;
...
always @(*) begin
    ...
    window_tap_x_i = TAP_ZERO;
    window_tap_y_i = TAP_ZERO;
    window_tap_result_i = {(PIXEL_W+2){1'b0}};
    win_idx_i = 0;
    //for产生8x8的窗口坐标，调用read_lanczos_tap函数读取像素值和有效性，组合成输出信号，后期需要验证是否可以正常使用。
    for (win_y_i = 0; win_y_i < LANCZOS_TAPS; win_y_i = win_y_i + 1) begin
        for (win_x_i = 0; win_x_i < LANCZOS_TAPS; win_x_i = win_x_i + 1) begin
            win_idx_i = win_y_i * LANCZOS_TAPS + win_x_i;
            window_tap_x_i = lanczos_center_x + tap_offset_by_idx(win_x_i[2:0]);
            window_tap_y_i = lanczos_center_y + tap_offset_by_idx(win_y_i[2:0]);
            window_tap_result_i = read_lanczos_tap(window_tap_x_i, window_tap_y_i);

            lanczos_window_pixels_r[win_idx_i*PIXEL_W +: PIXEL_W] = window_tap_result_i[PIXEL_W-1:0];
            lanczos_window_valid_mask_r[win_idx_i] = window_tap_result_i[PIXEL_W];
            lanczos_window_from_right_mask_r[win_idx_i] = window_tap_result_i[PIXEL_W+1];
        end
    end
end
...
assign tap_read_active = (cur_state == ST_LANCZOS_BUSY);
...
// ---------------------------------------------------------------------------
// Generic line SRAM write engine. Partial writes use read-modify-write.
// ---------------------------------------------------------------------------
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
...
// ---------------------------------------------------------------------------
// Read old line SRAM right 7 pixels before the row is overwritten.
//“从 right_rd_x 开始连续读 7 个像素”拆成 读当前 16 像素 word 和必要时 再读下一个 16 像素 word，然后把这 7 个像素按顺序写进
// ---------------------------------------------------------------------------
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (right_rd_state)
            RR_IDLE: begin
                if (right_rd_start) begin
                    ...
                    right_rd_cross_word <= (right_rd_x[3:0] > 4'd9);  //是否跨16像素边界
...
// ---------------------------------------------------------------------------
// Segment writeback sequencer: save old right edge, then RMW left7/current data.
//write_back阶段的状态机，也就是计算完一个lanczos后5
// ---------------------------------------------------------------------------
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        wb_state          <= WB_IDLE;
        ...
        if (cur_state == ST_FLUSH_RIGHT) begin  //如果当前是ST_FLUSH_RIGHT，说明 block 已经结束，需要补存最后 7 行的右边界。
            ...
            wb_state <= WB_IDLE;
        end else begin                                  //如果处于写回阶段
            case (wb_state)
                WB_IDLE: begin
                    if (save_evict_right_en) begin                          //需不需要保存右边界
                        right_rd_bank    <= calc_linebuf_row;
                        right_rd_x       <= right_base_x;
                        right_rd_dst_row <= evict_right_row_idx;
                        right_rd_start   <= 1'b1;
                        wb_state         <= WB_SAVE_RIGHT;
                    end else if (!calc_first_seg_in_row) begin              //是不是这行的第一个数据
                        line_wr_req_bank   <= calc_linebuf_row;
                        line_wr_req_x      <= calc_global_x_base - 13'd7;   //写回上一段暂存的7个像素
                        ...
                        line_wr_req_len    <= calc_last_seg_in_row ? cur_write_len : 5'd9;  //判断是不是一行最后一段
                        line_wr_req_pixels <= {LINEBUF_WORD_W{1'b0}};
